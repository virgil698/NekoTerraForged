package org.virgil698.NekoTerraForged.mixin.worldgen.continent.fancy;
...
public class FancyContinent implements RiverGenerator {
    ...
    public FancyContinent(int seed, int nodes, float radius, GeneratorContext context, FancyContinentGenerator continent) {
        ...
        this.islands = generateIslands(settings, 3, nodes, radius, new Random(seed));
    ...
    private static Island[] generateIslands(WorldSettings settings, int islandCount, int nodeCount, float radius, Random random) {
        int dirs = 4;
        Island main = generate(0, settings, nodeCount, radius, random);
        ...
        for (int dir = 0; dir < dirs; ++dir) {
            ...
            for (int island = 0; island < islandCount; ++island) {
                Island next = generate(i, settings, nCount, r, random);
    ...
    private static Island generate(int id, WorldSettings settings, int nodes, float radius, Random random) {
        ...
        return new Island(id, segments, settings, radius * 3.0f, radius * 1.25F, radius, radius * 0.975F);
    }
