 ...
 * 移植自 ReTerraForged
 */
public record ErosionNoise(Noise input, int seed, int octaves, float strength, float gridSize, 
                           float amplitude, float lacunarity, float distanceFallOff, BlendMode blendMode,
                           ThreadLocal<float[]> cache) implements Noise {

    public ErosionNoise(Noise input, int seed, int octaves, float strength, float gridSize, 
                        float amplitude, float lacunarity, float distanceFallOff, BlendMode blendMode) {
        this(input, seed, octaves, strength, gridSize, amplitude, lacunarity, distanceFallOff, blendMode, 
             ThreadLocal.withInitial(() -> new float[5 * 5]));
    }
    ...
    public Noise mapAll(Visitor visitor) {
        return new ErosionNoise(this.input.mapAll(visitor), this.seed, this.octaves, this.strength, 
            this.gridSize, this.amplitude, this.lacunarity, this.distanceFallOff, this.blendMode);
    }

    @Override
    public boolean equals(Object o) {
        return o instanceof ErosionNoise other && other.input.equals(this.input) && other.seed == this.seed 
            && other.octaves == this.octaves && other.strength == this.strength && other.gridSize == this.gridSize 
            && other.amplitude == this.amplitude && other.lacunarity == this.lacunarity 
            && other.distanceFallOff == this.distanceFallOff && other.blendMode.equals(this.blendMode);
    }
