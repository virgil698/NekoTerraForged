...
public class BiomeTypeLoader {
    ...
    public NoiseUtil.Vec2f[] getRanges(BiomeType type) {
        float minTemp = 1.0F;
        float maxTemp = 0.0F;
        float minMoist = 1.0F;
        float maxMoist = 0.0F;
        ...
        return new NoiseUtil.Vec2f[] { new NoiseUtil.Vec2f(minTemp, maxTemp), new NoiseUtil.Vec2f(minMoist, maxMoist) };
    }
    ...
    private void generateDefaultTypeMap() {
        // 生成默认的生物群系类型映射
        for (int y = 0; y < 256; ++y) {
            for (int x = 0; x < 256; ++x) {
                float temp = x / 255.0f;
                float moist = y / 255.0f;
                this.map[y][x] = BiomeType.getDefault(temp, moist);
            }
        }
    }
