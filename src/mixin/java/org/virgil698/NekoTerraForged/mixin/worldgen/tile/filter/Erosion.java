...
import org.virgil698.NekoTerraForged.mixin.worldgen.util.FastRandom;

/**
 * 水力侵蚀过滤器
 * 模拟水滴在地形上流动造成的侵蚀和沉积效果
 * 移植自 ReTerraForged
 */
public class Erosion implements Filter {
    ...
    public Erosion(int seed, int mapSize, ErosionSettings settings, Modifier modifier) {
        this.seed = seed;
    ...
    private static class Factory implements IntFunction<Erosion> {
        ...
        private Factory(int seed, Levels levels) {
            this.seed = seed + SEED_OFFSET;
            this.settings = ErosionSettings.defaults();
     ...
     * 侵蚀设置
     */
    public static class ErosionSettings {
        public float erosionRate = 0.5f;
        public float depositeRate = 0.3f;
        public float dropletVelocity = 1.0f;
        public float dropletVolume = 1.0f;
        public int dropletLifetime = 70;
        public int dropletsPerChunk = 150;

        public static ErosionSettings defaults() {
            return new ErosionSettings();
        }

        public ErosionSettings copy() {
            ErosionSettings copy = new ErosionSettings();
            copy.erosionRate = this.erosionRate;
            copy.depositeRate = this.depositeRate;
            copy.dropletVelocity = this.dropletVelocity;
            copy.dropletVolume = this.dropletVolume;
            copy.dropletLifetime = this.dropletLifetime;
            copy.dropletsPerChunk = this.dropletsPerChunk;
            return copy;
        }
    }
}
