...
import org.virgil698.NekoTerraForged.mixin.worldgen.biome.Humidity;
import org.virgil698.NekoTerraForged.mixin.worldgen.biome.Temperature;
...
public enum BiomeType {
     ...
     * 默认的生物群系类型选择（不依赖图片）
     */
    public static BiomeType getDefault(float temperature, float moisture) {
        if (temperature >= 0.8F) {
            if (moisture >= 0.6F) return TROPICAL_RAINFOREST;
            if (moisture >= 0.3F) return SAVANNA;
            return DESERT;
        } else if (temperature >= 0.5F) {
            if (moisture >= 0.7F) return TEMPERATE_RAINFOREST;
            if (moisture >= 0.4F) return TEMPERATE_FOREST;
            if (moisture >= 0.2F) return STEPPE;
            return GRASSLAND;
        } else if (temperature >= 0.25F) {
            if (moisture >= 0.5F) return TAIGA;
            return COLD_STEPPE;
        } else {
            if (moisture >= 0.7F) return ALPINE;
            return TUNDRA;
        }
    }
    ...
    private static void init() {
        for (BiomeType type : values()) {
            NoiseUtil.Vec2f[] ranges = BiomeTypeLoader.getInstance().getRanges(type);
