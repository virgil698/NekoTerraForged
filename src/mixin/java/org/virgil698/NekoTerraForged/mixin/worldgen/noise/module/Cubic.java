...
public record Cubic(float frequency, int octaves, float lacunarity, float gain, float minValue, float maxValue) implements Noise {
    ...
    public static float sample(float x, float y, int seed) {
        ...
        return NoiseUtil.cubicLerp(
            NoiseUtil.cubicLerp(NoiseUtil.valCoord2D(seed, x3, y3), NoiseUtil.valCoord2D(seed, x2, y3), NoiseUtil.valCoord2D(seed, x4, y3), NoiseUtil.valCoord2D(seed, x5, y3), xs),
            NoiseUtil.cubicLerp(NoiseUtil.valCoord2D(seed, x3, y2), NoiseUtil.valCoord2D(seed, x2, y2), NoiseUtil.valCoord2D(seed, x4, y2), NoiseUtil.valCoord2D(seed, x5, y2), xs),
            NoiseUtil.cubicLerp(NoiseUtil.valCoord2D(seed, x3, y4), NoiseUtil.valCoord2D(seed, x2, y4), NoiseUtil.valCoord2D(seed, x4, y4), NoiseUtil.valCoord2D(seed, x5, y4), xs),
            NoiseUtil.cubicLerp(NoiseUtil.valCoord2D(seed, x3, y5), NoiseUtil.valCoord2D(seed, x2, y5), NoiseUtil.valCoord2D(seed, x4, y5), NoiseUtil.valCoord2D(seed, x5, y5), xs),
            ys
        ) * 0.44444445f;
    }
